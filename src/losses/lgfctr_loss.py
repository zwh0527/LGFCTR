...
class LGFCTRLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config  # config under the global namespace
        self.loss_config = config['LGFCTR']['LOSS']
        self.match_type = self.config['LGFCTR']['MATCH_COARSE']['MATCH_TYPE']
        self.sparse_spvs = self.config['LGFCTR']['MATCH_COARSE']['SPARSE_SPVS']
        ...
        self.f_pos_w = self.loss_config['POS_WEIGHT_CLS']
        self.f_neg_w = self.loss_config['NEG_WEIGHT_CLS']
    ...
    def compute_fine_loss(self, expec_f, expec_f_gt):
        if self.fine_type == 'l2_with_std' or self.fine_type == 'l2 + cls':
    ...
    def compute_fine_loss_cls(self, prob_f, expec_f_gt):
        """
        Args:
            prob_f (torch.Tensor): [M, 2] <pos_prob, neg_prob>
            expec_f_gt (torch.Tensor): [M, 2] <x, y>
        """
        # correct_mask tells you which pair to compute fine-loss
        pos_mask = torch.linalg.norm(expec_f_gt, ord=float('inf'), dim=1) < self.correct_thr
        neg_mask = ~pos_mask
        f_pos_w, f_neg_w = self.f_pos_w, self.f_neg_w
        ...
        prob_f = torch.clamp(prob_f, 1e-6, 1 - 1e-6)
        loss_pos = - torch.log(prob_f[pos_mask])
        loss_neg = - torch.log(1 - prob_f[neg_mask])
        loss = f_pos_w * loss_pos.mean() + f_neg_w * loss_neg.mean()
        return loss
    ...
    def forward(self, data):
        ...
        # 3. fine_level cls loss
        if self.fine_type == 'l2 + cls':
            loss_f_cls = self.compute_fine_loss_cls(data['prob_f'], data['expec_f_gt'])
            if loss_f_cls is not None:
                loss += loss_f_cls * self.loss_config['FINE_WEIGHT_CLS']
                loss_scalars.update({"loss_f_cls": loss_f_cls.clone().detach().cpu()})
            else:
                assert self.training is False
                loss_scalars.update({'loss_f_cls': torch.tensor(0.7)}) # log(2)
