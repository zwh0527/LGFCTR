...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        self.mgdpt_img_resize_val = config.DATASET.MGDPT_IMG_RESIZE_VAL # 1200
        self.mgdpt_img_pad = config.DATASET.MGDPT_IMG_PAD   # True
        self.mgdpt_depth_pad = config.DATASET.MGDPT_DEPTH_PAD   # True
        self.mgdpt_df = config.DATASET.MGDPT_DF  # 8
        self.coarse_scale = 1 / config.LGFCTR.RESOLUTION[0]  # 0.125. for training LGFCTR.
    ...
    def _build_concat_dataset(
        ...
        for npz_name in tqdm(npz_names,
            ...
            if data_source == 'ScanNet':
                ...
                # 此处为了在val时直接对1200进行测试，稍微进行了微调
                img_resize = self.mgdpt_img_resize_val if mode == 'val' else self.mgdpt_img_resize
                datasets.append(
